...
**本次落地说明：** FastAPI 的轨迹读取响应已回传既有 `clientUuid`，无需数据库迁移。iOS 新增 `FootprintRepository` 与 `FootprintDisplayPoint`，以稳定幂等键合并远端快照、`UserDefaults` 待传样本及本次运行内已确认样本，服务端读回后再由远端事实接管。`LocationTrackingStore` 保持既有批量上传职责，并补充单一在途同步、20 点/30 秒调度、前台恢复、停止记录和有限退避重试；业务错误不会阻塞其他 Trip。会话恢复在临时断网时保留 Keychain 凭证并使用最小本地资料、最近行程摘要恢复离线地图，联网后仍以服务端结果校验。离线阶段可继续采样和渲染本地轨迹；手动打卡仍为在线写入，未新增离线打卡队列。
...
实施状态：已完成（2026-09-13，完成采集端质量过滤、显示层过滤与推断断段，并以可选 trackingSegmentUuid 持久化停止/恢复边界）
